...
/* ── Field validation (delegates to ilink3_auth.h) ───────────────────────── */

static int validate_client_fields(const demo_session_t *s)
{
    ilink3_fields_t f;
    memset(&f, 0, sizeof(f));
    strncpy(f.access_key, s->access_key, sizeof(f.access_key) - 1);
    strncpy(f.session_id, s->session_id, sizeof(f.session_id) - 1);
    strncpy(f.firm_id,    s->firm_id,    sizeof(f.firm_id)    - 1);
    strncpy(f.interface,  s->interface,  sizeof(f.interface)  - 1);
    strncpy(f.app_name,   s->app_name,   sizeof(f.app_name)   - 1);
    strncpy(f.app_ver,    s->app_ver,    sizeof(f.app_ver)    - 1);
    strncpy(f.app_vendor, s->app_vendor, sizeof(f.app_vendor) - 1);
    return validate_fixed_fields(&f);
}
